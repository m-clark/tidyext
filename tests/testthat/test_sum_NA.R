...
df_nan <- df1 %>%
  mutate_at(dplyr::vars(a,b),
            function(x) {x[sample(seq_along(x), 5)] <- NaN; x})

df_blank <- df1 %>%
  mutate_at(dplyr::vars(c),
            function(x) {x[sample(seq_along(x), 5)] <- ''; x})
...
test_that('sum_NaN correctly returns value', {
  expect_equal(df_nan %>%
                 summarise_at(dplyr::vars(a,b), sum_NaN) %>%
                 sum(), 10)
})
...
test_that('sum_NaN fails if not a numeric', {
  expect_error(df1 %>%
                 summarise_at(dplyr::vars(g1), sum_NaN) %>%
                 sum())
})

test_that('sum_blank correctly returns value', {
  expect_equal(df_blank %>%
                 summarise_at(dplyr::vars(c), sum_blank) %>%
                 sum(), 5)
})
